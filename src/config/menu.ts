...
export const adminMenu = [
  ...
  },
  {
    ...
    icon: Settings,
    children: [
      { title: '账号管理', path: '/system/accounts', icon: Users },
      { title: '角色管理', path: '/system/roles', icon: UserCog },
      { title: '组织管理', path: '/system/orgs', icon: Network },
      { title: '基础配置', path: '/system/config', icon: Sliders },
      { title: '菜单管理', path: '/system/menus', icon: Menu },
      { title: '登录日志', path: '/system/login-logs', icon: LogIn },
      { title: '操作日志', path: '/system/op-logs', icon: MousePointerClick },
      { title: '短信记录', path: '/system/sms', icon: MessageSquare },
    ],
  },
];
